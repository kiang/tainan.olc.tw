...
locateCoordsBtn.addEventListener('click', function() {
    ...
    // Wait for the map to finish moving, then fire the click event
    map.once('moveend', function() {
        // Ensure the map click simulation uses the most up-to-date layer/container points
        // after the map has settled at the new view.
        const currentLayerPoint = map.latLngToLayerPoint(latlng);
        const currentContainerPoint = map.latLngToContainerPoint(latlng);

        map.fire('click', {
            latlng: latlng,
            layerPoint: currentLayerPoint,
            containerPoint: currentContainerPoint,
            originalEvent: null // Indicate it's a synthetic event, though not strictly necessary for current handler
        });
    });
