...
map.on('singleclick', function (evt) {
  ...
  setTimeout(() => {
    if (false === pointClicked) {
      ...
      sidebarTitle.innerHTML = '提供新的店家';
      currentFeature = false;
      if (false !== previousFeature) {
        previousFeature.setStyle(pointStyleFunction(previousFeature));
      }
      $('#sidebarContent').append('<iframe src="https://docs.google.com/forms/d/e/1FAIpQLSebXUt27kmQswXEpqIN2Wn06YEH9TRuv9o7W2AZc4QzhByrOA/viewform?usp=pp_url&entry.953842169=&entry.424539609=&entry.1998738256=' + clickedCoordinate[0] + '&entry.1387778236=' + clickedCoordinate[1] + '&entry.2072773208=' + uuidv4() + '&embedded=true" height="1200" frameborder="0" marginheight="0" marginwidth="0" style="width: 100%;">Loading…</iframe>');
...
function showPoint(pointId) {
  ...
  for (k in features) {
    ...
    if (p.id === pointId) {
      ...
      message += '<tr><th scope="row">住址門牌</th><td>' + p.address + '</td></tr>';
      message += '<tr><th scope="row">情況描述</th><td>' + p.desc + '</td></tr>';
      ...
      $('input#btn-update').click(function (e) {
        ...
        e.preventDefault();
        $(this).parent().html('<iframe src="https://docs.google.com/forms/d/e/1FAIpQLSebXUt27kmQswXEpqIN2Wn06YEH9TRuv9o7W2AZc4QzhByrOA/viewform?usp=pp_url&entry.1868104380=' + p.name + '&entry.1173058939=' + p.address + '&entry.2066572285=' + p.desc + '&entry.1998738256=' + p.longitude + '&entry.1387778236=' + p.latitude + '&entry.2072773208=' + p.id + '&embedded=true" height="1200" frameborder="0" marginheight="0" marginwidth="0" style="width: 100%;">Loading…</iframe>');
      });
    }
  }
  sidebar.open('home');
...
$.get('https://docs.google.com/spreadsheets/d/e/2PACX-1vRYRvXIw_pKzcOPdffq2PcBmrV6csCWXshvByOb4UMBIBdfo6Uw4eHCnXmjjU4y2bjZ55lEOdrEtsOn/pub?output=csv', {}, function (c) {
  var lines = $.csv.toArrays(c);
  lines.shift();
  for (k in lines) {
    lines[k][5] = parseFloat(lines[k][5]);
    lines[k][6] = parseFloat(lines[k][6]);
    if (!lines[k][6] || Number.isNaN(lines[k][5]) || Number.isNaN(lines[k][6])) {
      continue;
    }
    var key = lines[k][7];
    var status = 1;
